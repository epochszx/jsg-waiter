  ...
  <div class="order_item">
    <van-row class="header"> 
      ...
      <van-col span="12" class="status">
          <van-button @click="orderAcceptHandler(data.id)"  plain type="info" size="mini">接 受</van-button>
          <van-button @click="orderRejectHandler(data.id)" plain type="danger" size="mini">拒 绝</van-button>
      </van-col>
    </van-row>
    <van-row>
      <van-col :span="6">
...
export default {
  ...
  methods:{
      ...mapActions('order',['acceptOrder','rejectOrder','queryOrder']),
      orderAcceptHandler(id){
           this.$toast.loading({
                message: '加载中...',
                forbidClick: true,
                loadingType: 'spinner',
